...
- [x] Prozess-/Container-Leiste: `ProcessSupervisor.health()` + Docker-Stats
      als kompakte Leiste (= ctop-UI-Slice), Restart-Ereignisse markiert. <!-- done (BEIDE Hälften): (1) `ContainerStrip` — pro Container Status-Dot/Name/Image/CPU%/Mem/Ports aus dem `RuntimeProvider`-Stream (`fakeRuntimeProvider`), CPU/Mem-Ampel; (2) `ProcessStrip` — pro supervised Prozess (PTY/LSP/DAP/agent) State-Dot/id/pid/Restarts aus einem NEUEN `SupervisorProvider`-Frontend-Contract (`contracts/supervisor.ts`, spiegelt die sidecar `ProcessHealth`-Form: nur Lifecycle-Fakten, kein command/output/secret) + `mockSupervisorProvider`; Restart-Ereignisse via ↺-Marker (Warning-Ton). Beide Strips key-sortiert → deterministisch/golden-stabil; null-cost subscribe-on-mount/unsubscribe-on-unmount; `.matrix-graph-scroll` als fokussierbare Region (axe scrollable-region-focusable). DEFERRED nur noch: die reale IPC-Verdrahtung `ProcessSupervisor` (sidecar) → `SupervisorProvider` (Klasse-B, real-runtime — der Contract+Mock sind der dünne Swap-Seam). -->
...
- [~] Design-Dokument Scoped-Grant-UX (gated: erst Review, dann Bau):
      ...
      Allowlist, kein Wildcard-Default. <!-- Entwurf gelandet: agents/design-gates/scoped-grant-ux.md (autonom gezeichnet, geerdet an policy-engine.ts/broker.ts: Grant-Shape mit realpath-kanonisiertem pathPrefix + Task-Bindung als scoped-Variante ohne neue Achse, Threat-Model A1–A7 deckt Z. 232 ab, Gate-pro-Aktion-Matrix, offene Fragen Q1–Q5). GATE-STATUS jetzt ⛔ FAIL (Council-Pre-Review 2026-07-06, 3 adversariale Linsen, 2× FAIL/1× CONDITIONAL): fand strukturelle Bypässe + einen latenten Security-Bug im shipped Code (untrusted `shell` umging das Trifecta-Gate — in derselben Runde gefixt: `shell` zu EGRESS_KINDS + Regressions-Tests). 9 Blocker im Design zu adressieren (Scope unverdrahtet, Symlink-Write, Task-Bindung aspirational, Revoke-Race, maxActions mandatory, …) vor Matzes Review + Bau. UPDATE 2026-07-06: v2-Auflösung aller 9 Blocker + frische Re-Review-Runde (2 Linsen) → 🟡 v2.1 CONDITIONAL: A1/A2-Root-Boundary bestätigt geschlossen, kein Escape mehr — nur Wiring-Specs offen (canonical-Target, Ordering, taskId-Durchreichung, Res-7-neu, Case-Fold), in § v2.1 festgeschrieben. Wartet auf Matze-Sign-off (oder 3. Runde). Siehe scoped-grant-ux.md § v2 Re-Review. -->
...
- [~] Design-Schritt Headless-Lifecycle (gated vor den Bau-Schritten):
      ...
      Ressourcen-Limits (max. Laufzeit, Token-Budget pro Lauf). <!-- Entwurf gelandet: agents/design-gates/headless-lifecycle.md (autonom gezeichnet, geerdet an stall-watchdog.ts/reconnecting-client.ts/in-memory-session-store.ts: 6 Terminierungs-Kriterien, ehrliche Persistenz-Tabelle (nur der Digest übersteht die Nacht), Ressourcen-Limits, Pausiert-nie-eskaliert-Invariante). GATE-STATUS jetzt ⛔ FAIL (Council-Pre-Review 2026-07-06, Vollständigkeits-Linse): von 6 Terminierungs-Kriterien hat nur Idle-Timeout Code; „pausiert-nie-eskaliert" ist widerlegt (reales `ask`-Gate auto-denyt nach 5 min); Max-Laufzeit + Token-Budget + Digest-Redaktion fehlen. GAP-1/2/5 gate-blockierend. Vor Matzes Review + Bau zu adressieren; blockiert weiter auf P1. UPDATE 2026-07-06: v2-Auflösung aller GAPs + Re-Review → 🟡 v2.1 CONDITIONAL: GAP-1/2/4/6 schließbar, GAP-5 (Digest ohne Freiform-`target`) + GAP-1 (Pause-statt-Hang/Resume) + Epoch-Storage in § v2.1 geschärft. Wartet auf Matze-Sign-off + P1-Landung. Siehe headless-lifecycle.md § v2 Re-Review. -->
