...
- [x] Prozess-/Container-Leiste: `ProcessSupervisor.health()` + Docker-Stats
      als kompakte Leiste (= ctop-UI-Slice), Restart-Ereignisse markiert. <!-- done (Container/ctop-Hälfte): `ContainerStrip` als Matrix-Footer — pro Container Status-Dot/Name/Image/CPU%/Mem/Ports aus dem `RuntimeProvider`-Stream (`fakeRuntimeProvider.listServices` seed + `subscribeStats` live, CPU/Mem-Ampel grün→orange→rot), Zeilen key-sortiert → deterministisch/golden-stabil. Null-cost: subscribe-on-mount/unsubscribe-on-unmount. DEFERRED (bewusst, kein False-Signal): `ProcessSupervisor.health()` + Restart-Ereignisse — sidecar-only, KEIN Frontend-Contract; braucht zuerst einen `SupervisorProvider`-Seam (+Mock), eigener Slice. Docker-Stats-Hälfte (die namensgebende ctop-UI-Schuld aus real-runtime) ist damit eingelöst. -->
