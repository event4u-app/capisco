...
- [ ] Broker-Ticker + Audit-Log-Viewer (löst actually-works P3 Z. 155 ein):
      ...
      werden gerendert (`env.API_KEY`), nie Werte. <!-- next PR: GATED — `AuditStore`-Contract existiert (`contracts/broker.ts`), aber KEIN Frontend-Mock + kein `subscribe` über die IPC-Wire (`BrokerProvider` bietet nur `listAudit()`-Snapshot). Braucht zuerst `MockAuditStore` (deterministisch, `credentialRef` = Name, nie Wert) + ggf. `AuditProvider`-Seam. Bewusst NICHT als Fake-Zeile gerendert (kein False-Signal). -->
