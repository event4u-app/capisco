...
- [ ] **Checkpoint-/Branch-Navigation (S8):** „Checkpoint" benennt den aktuellen `SessionTree`-Leaf; kompakter Branch-Switcher springt zwischen divergenten Prompt-Linien. Baut auf Edit-&-Rerun-Branch auf. <!-- next PR: baut auf dem gelandeten Edit-&-Rerun-Branch auf (eigene `checkpoints`-Slice + `BranchSwitcher` in `.cmp-belowbar`, gated auf Tree-Divergenz → boot-invisible). -->
