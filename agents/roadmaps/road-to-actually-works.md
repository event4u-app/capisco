...
- [ ] **Umbenennbare Tabs**, `+`/schließen, Split/Kill — an echte PTYs. <!-- Tabs/`+`/Close/Rename/Kill jetzt an echte Per-Tab-PTYs gebunden (xterm in Terminal.tsx, close→provider.close, je Tab eigene Shell+Scrollback); Wiring unit-verifiziert. NUR Split (Pane-Layout, zwei PTYs nebeneinander) fehlt noch → bleibt offen. -->
