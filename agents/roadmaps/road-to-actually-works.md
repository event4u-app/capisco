...
- [x] **Audit-Log-Viewer:** der append-only Audit-Strom (Akteur + Capability +
      credentialRef, nie Wert) sichtbar in der UI. <!-- done: `BrokerTicker` (app/src/shell/matrix/BrokerTicker.tsx) rendert den Live-Audit-Strom als read-only Projektion in der Matrix-Mode-UI — Akteur + Capability + Entscheidung + optionaler `credentialRef`-NAME. Secret-safe by construction: `AuditEntry` hat kein value-Feld, ein Secret-Wert kann nie gerendert werden. Abonniert `AuditStore.subscribe` (der [x] Broker-Entscheidungs-Stream unten) on-mount, unsubscribe on-unmount (null-cost-Invariante). Verifiziert: BrokerTicker.test.tsx (Render + Expand-zum-Trail + credentialRef-statt-Wert) grün in der Vitest-Suite. -->
- [ ] **[gegated] IPC-Streaming-Bridge Sidecar→Client:** der Audit-Stream läuft heute in-process/über den Contract-Mock; die native `subscribe`-Bridge über den echten Tauri-IPC-Transport ist ein consumer-side Folge-Slice (downstream von WS→Tauri-IPC-Swap). <!-- deferred: der Viewer + der In-Memory-Stream sind gebaut & verifiziert; nur der reale IPC-Transport fehlt (road-to-real-runtime / native shell). -->
