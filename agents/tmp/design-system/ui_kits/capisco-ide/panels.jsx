      ...
      {tab === 'changes' ? (
        <>
          ...
          <div className="ws-commit">
            <textarea className="ws-commit-msg" placeholder="Commit message…" defaultValue=""></textarea>
...
function Terminal() {
  ...
  const close = (t) => { setTabs((ts) => ts.filter((x) => x !== t)); setActive((a) => (a === t ? (tabs.find((x) => x !== t) || '') : a)); };
  return (
    <div className="terminal">
      <div className="term-tabbar">
        <div className="term-tools">
          <span className="term-label"><Icon name="square-terminal" size={14} color="var(--text-secondary)" /></span>
          <IconButton size={22} icon={<Icon name="trash-2" size={13} />} title="Kill" />
        </div>
        <div className="term-tabs">
          {tabs.map((t) => (
            <button key={t} className={'term-tab' + (t === active ? ' active' : '')} onClick={() => setActive(t)}>
              {t}<span className="term-x" title="Close" onClick={(e) => { e.stopPropagation(); close(t); }}>×</span>
            </button>
          ))}
