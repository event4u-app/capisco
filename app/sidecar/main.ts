...
/**
 * Construct a sidecar with all providers registered (not yet listening). The
 * Recent-Projects provider is the real **file-backed** machine-wide registry;
 * every other provider is its deterministic mock (real adapters are a thin swap
...
export function createSidecar(socketPath: string): Sidecar {
  ...
  registerMockProviders(sidecar.registry, recent);
